...
class DataManager:
    ...
    def get_items(self, url, gui_options, use_cache=False):
        ...
        log.debug("last_content_url : use_cache=%s url=%s", use_cache, url)
            ...
            with FileLock(cache_file + ".locked", timeout=5):
                with open(cache_file, 'rb') as handle:
                    try:
                        ...
                        log.error("Pickle Data Load Failed : %s", err)
                        item_list = None
...
class CacheManagerThread(threading.Thread):
    ...
    def run(self):
        ...
        if is_fresh and self.cached_item.item_list is not None and len(self.cached_item.item_list) > 0:
            ...
            log.debug("CacheManagerThread : Cache Hash : %s", cached_hash)
            ...
            log.debug("CacheManagerThread : Loaded Hash : %s", loaded_hash)
...
def clear_cached_server_data():
    ...
    for filename in files:
        if filename.startswith("cache_") and filename.endswith(".pickle"):
            log.debug("Deleting CacheFile: %s", filename)
            xbmcvfs.delete(os.path.join(addon_dir, filename))
            del_count += 1

    log.debug('Deleted %s files', del_count)
...
def clear_old_cache_data():
    ...
    for filename in files:
        if filename.startswith("cache_") and filename.endswith(".pickle"):
            log.debug("clear_old_cache_data() : Checking CacheFile : %s", filename)
            ...
            for x in range(0, 5):
                try:
                    ...
                    log.debug("clear_old_cache_data() : Pickle load error : %s", error)
                    cache_item = None
                    xbmc.sleep(1000)
            ...
            if cache_item is not None:
                ...
                log.debug("clear_old_cache_data() : Cache item last used : %s sec ago", item_last_used)
                if item_last_used == -1 or item_last_used > (3600 * 24 * 7):
                    log.debug("clear_old_cache_data() : Deleting cache item age : %s", item_last_used)
                    data_file = os.path.join(addon_dir, filename)
                    with FileLock(data_file + ".locked", timeout=5):
    ...
    log.debug("clear_old_cache_data() : Cache items deleted : %s", del_count)
